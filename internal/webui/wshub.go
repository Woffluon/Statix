	...
	for {
		...
		case snapshot := <-h.broadcast:
			...
			for client := range h.clients {
				...
				default:
					h.logger.Warn("wshub: client send buffer full, dropping snapshot")
				}
			}
			h.mu.RUnlock()
		}
...
func (h *WSHub) UpgradeAndServe(w http.ResponseWriter, r *http.Request, store *auth.SessionStore) {
	...
	conn, err := websocket.Accept(w, r, &websocket.AcceptOptions{
		InsecureSkipVerify: true, // Allow connection from local app origin
	})
	if err != nil {
	...
	client := &wsClient{
		conn:   conn,
		send:   make(chan metrics.Snapshot, 4),
		ctx:    ctx,
		cancel: cancel,
	}
