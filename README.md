...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Docker-Ready-000000?style=for-the-badge&logo=docker&logoColor=white" alt="Docker Ready">
</p>
...
│  │            /healthz    /setup    /login    /dashboard    /ws            │ │
│  └─────────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
| **Container Ready** | **Yes** (Native Docker & Compose) | Yes | Yes |
...
Single-file Zero-Clone Docker Compose snippet for Portainer, Coolify, or any Linux host:
...
services:
  statix:
    image: statix:latest
    build: https://github.com/Woffluon/Statix.git#main
    container_name: statix
    restart: unless-stopped
    ports:
      - "8080:8080"
    volumes:
      - statix-config:/etc/statix
      - /proc:/host/proc:ro
...
Run in one command on server:

```bash
mkdir -p /opt/statix && cat << 'EOF' > /opt/statix/docker-compose.yml
version: '3.8'
...
services:
  statix:
    ...
    ports:
      - "8080:8080"
    volumes:
      - /opt/statix/config:/etc/statix
      - /proc:/host/proc:ro
EOF
cd /opt/statix && docker compose up -d
```

---

### Option 2: Standalone Systemd Installer
...
The script automatically detects CPU architecture (`amd64` / `arm64`), downloads the binary release, verifies SHA256 checksums, provisions an unprivileged `statix` system user, and activates the hardened systemd unit.
...
- **Cookie Security:** Cookies set with `HttpOnly` and `SameSite=Strict`.
...
### Prerequisites
- Go 1.22 or higher
- Linux host, WSL (Ubuntu), or Docker
